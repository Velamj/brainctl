...
**Issue:** internal-ref  
**Date:** 2026-04-02  
**Agent:** Weaver (task-tracker-weaver)  
...
| Score    | Node          | Label                                                    |
|----------|---------------|----------------------------------------------------------|
| 0.005390 | memories#78   | example-app invoice subsystem note                         |
| 0.004136 | memories#93   | Agent memory spine state (2026-03-28)                    |
| 0.004095 | memories#127  | brainctl push gate threshold                             |
| 0.004095 | memories#130  | example-app stack description                           |
| 0.004093 | memories#125  | Kernel brainctl integration                     |
| 0.004071 | context#189   | Routed context from event:86                     |
| 0.004071 | context#188   | Routed context from event:86                     |
| 0.004071 | context#183   | internal-ref: Resolved stale_assumption findings               |
| 0.003907 | events#187    | brain_tool.py integration complete              |
| 0.003735 | events#186    | test event                                               |
| 0.003699 | events#191    | Cortex heartbeat 3: closed internal-ref                       |
| 0.003472 | memories#106  | Compressed 15 memories (example-app:invoice)            |

**Findings:**
- memories#78 (example-app invoice subsystem) is the single most central node — appropriately a project-critical compound memory
...
- Context nodes cluster near events from the same internal-ref coherence run — PageRank detected the dense cross-referencing created by that activity burst
...
| 23        | 21    | memories#78, #55, #36 — example-app-focused memories          |
...
- Community 23 (example-app memories centered on memories#78) validates that the highest-PageRank node anchors its own semantic community
- Community 2 (Kernel brainctl) is the largest memory community — reflects dense cross-referencing during internal-ref integration work
...
| 0.030273 | events#20   | internal-ref: hippocampus-cycle.sh created                     |
| 0.027487 | events#146  | Distilled event#93 → memory#108                          |
| 0.015776 | events#163  | Wave 2+4 research complete                               |
| 0.015043 | events#126  | internal-ref closed as duplicate of internal-ref                     |
| 0.014067 | events#38   | Cadence metrics refreshed                                 |
| 0.013843 | events#31   | Cadence metrics refreshed                                 |
| 0.011517 | memories#78 | example-app invoice subsystem                               |
| 0.009520 | events#123  | internal-ref: distillation policy drafted                       |
| 0.009520 | events#122  | internal-ref: situation model research                         |
...
Shortest path: 1 hops
  [0] memories#78  example-app invoice subsystem...
...
- Query "example-app invoice": memories#78 (highest PageRank) promoted above memories#130 at alpha=0.3
- Without boost: memories#130 (example-app stack description, more FTS matches) ranks first
...
**Cache strategy:** All results stored in `agent_state` under `agent_id='task-tracker-weaver'`. Force-refresh with `--force`. TTL: 24h for PageRank/communities, 48h for betweenness.
...
- : Knowledge Graph Layer — DONE
- : Spreading Activation — DONE
- : Dynamic edge weights (neuroplasticity) — DONE
- : Global Workspace broadcast — DONE
- : Quantum Walk on Knowledge Graph — DONE (research only)
- : Advanced Retrieval & Reasoning — found PageRank reranking to be highest-ROI graph enhancement
