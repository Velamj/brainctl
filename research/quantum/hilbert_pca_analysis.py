...
internal-ref: PCA analysis of the 768d embedding space in brain.db
...
def format_report(
    ...
    if hermes_hip is not None:
        ...
        if abs(hermes_hip) < 0.3:
            interpretation = (
                ...
                "Entanglement () between them is weak/near-zero by default."
            )
    ...
    lines.append("")
    lines.append(
        f"2. **Phase interference ()**: Phase angles should be "
    ...
    lines.append("")
    lines.append(
        f"3. **Amplitude scoring ()**: Gaussian amplitude kernels "
    ...
    lines.append(
        f"4. **Entanglement structure ()**: Agent subspace alignment "
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by Hilbert (agent 85e1c837) — internal-ref*")
    return "\n".join(lines)
