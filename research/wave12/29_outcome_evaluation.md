...
**Date:** 2026-03-28
**Issue:** internal-ref — [Research-W12] Outcome-Linked Memory Evaluation
**Status:** Complete
...
After 11 waves of cognitive architecture investment, we lack a validated measurement of whether enhanced memory retrieval produces better agent task outcomes. This report designs an outcome-linked evaluation framework that connects memory operations to measurable task-tracker task signals. The framework is grounded in established AI evaluation literature, adapted for the specific operational constraints of the brainctl/task-tracker system.
...
**Relevance to this system:** brainctl now has `infer-pretask` (pre-task inference) and `agent_uncertainty_log` . This aligns exactly with Goel's metacognitive evaluation loop:
...
**Relevance to this system:** We now have `brier_score` in `agent_expertise`  and source-weighted confidence at write time. BIG-Bench's calibration framework gives us a direct protocol:
...
2. **System metrics** — task completion rate, cycle time, escalation rate (task-tracker exposes these)
3. **Business metrics** — ROI per engineering hour, error rate, rework (derivable from task-tracker data)

The gap in our current system is tier 2 and tier 3. This report closes it.

---

## 2. task-tracker Signal Inventory

task-tracker exposes the following measurable outcome signals per task:
...
- `confidence` × `trust_score` (from source-weighted writes, internal-ref)
...
Pull task signals from task-tracker and cross-reference with brainctl access log.

```
Output:
  task_id:          internal-ref
  agent:            task-tracker-cortex
  cycle_time_hrs:   1.2
  heartbeat_count:  2
  blocked_count:    0
  memory_recalls:   7
...
Output:
  ...
  1. task-tracker-recall:    +31% lift, 89 tasks, calibration=0.09
  2. task-tracker-cortex:    +22% lift, 24 tasks, calibration=0.12
  3. task-tracker-sentinel-2: +18% lift, 31 tasks, calibration=0.14
  ...
  Fleet avg: +21% lift
...
- task-tracker task data pulled via existing API (`GET /api/issues/{id}`)
...
1. Select a task type that repeats regularly (e.g., internal-ref weekly updates, routine research tasks)
...
Given the Recall Gini RED alert and the active Attention Budget System , the quasi-experimental approach is preferable. Recommended design:
...
- **Timeline:** Can run immediately on historical data in brain.db + task-tracker API
...
## 6. Recommended Metrics for internal-ref (Cognitive Evolution Log)
...
| **Fleet avg cycle_time_hrs** | ≤3h | >6h | task-tracker API |
...
| **Escalation rate** | ≤15% | >30% | task-tracker API |
...
*This report was produced by research-agent for the Cognitive Architecture & Enhancement project. Linked issues: , , , .*
