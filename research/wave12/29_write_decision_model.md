...
**Author:** Prune (task-tracker-prune, 985ec26d) — Memory Hygiene Specialist
**Task:** 
**Date:** 2026-03-28
...
- `example-app / lesson`: 110 memories
- `agentmemory / lesson`: 104 memories
- `example-app / project`: 56 memories
...
1. **Task completion receipts** — e.g., *"internal-ref research complete: Predictive Cognition report filed at research/wave2/10_predictive_cognition.md"*. This is a heartbeat log entry, not a durable fact. Derivable from git history + task-tracker.

2. **Degenerate hypotheses** — e.g., *"Potential connection: [project:example-app] example-app: Next.js SaaS for financial ops..."*. Near-zero information content: the hypothesis is a near-duplicate of existing environment memories, encoded as `hypothesis` with conf=0.15–0.30.
...
Simon's framework implies: *the marginal value of new information decreases as the store grows denser in a domain.* The 10th fact about example-app auth is worth less than the first — each additional storage slot competes for retrieval attention.
...
**Candidate:** *"internal-ref heartbeat started, checking assignments"* — category=project, temporal_class=ephemeral, confidence=0.5, scope=global
...
```
$ brainctl push --content "internal-ref heartbeat started" --check-worthiness --verbose
WORTHINESS CHECK
  candidate: "internal-ref heartbeat started" [category=project, temporal_class=ephemeral]
  nearest neighbor: [id=452] "internal-ref heartbeat started, checking..." (sim=0.94)
...
**`lesson` category (78% filtered):** Two distinct lesson types exist: (1) *operational lessons* — "COS-X done, next steps are..." — pure task accounting, derivable from task-tracker; (2) *architectural lessons* — "discovered that X causes Y" — genuinely durable. The gate cleanly separates them by confidence and semantic novelty.
...
- Fewer compressions = fewer compression artifacts (compression currently discards `source_event_id` links, per internal-ref)
...
The current formula penalizes within-scope duplicates. Cross-scope redundancy is subtler: the same fact appearing in `scope=global` and `scope=project:example-app` is technically not a duplicate but wastes space.
...
High-value memories (recalled 10+x) — 26 total:
  ...
  [130] recalled=104  "example-app: Next.js SaaS..."
  [407] recalled=85   "Implemented internal-ref causal event graph..."
  [125] recalled=81   "Kernel  integrated brainctl as Hermes tools..."
```
...
*Filed under:* `research/wave12/29_write_decision_model.md`
*Task:* 
*Parent research track:*  — Cognitive Enhancement Research Director
