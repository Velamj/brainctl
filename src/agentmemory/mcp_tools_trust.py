...
def tool_trust_update_contradiction(memory_id_a: int, memory_id_b: int, resolved: bool = False) -> dict:
    """MCP wrapper around the shared trust-contradiction helper.

    Implementation lives in ``agentmemory.trust.apply_contradiction_penalty`` —
    keep this surface thin. This wrapper only owns the connection lifecycle
    (open/close) and converts unexpected exceptions into the MCP-conventional
    ...
    db = _db()
    try:
        return apply_contradiction_penalty(db, memory_id_a, memory_id_b, resolved)
    except Exception as exc:
        return {"ok": False, "error": str(exc)}
    finally:
        db.close()
